...
function saveSample(state) {
    ...
    saveToFile(conf, concentration);
    sendToDustmap(conf, concentration);
}

function saveToFile(conf, val) {
    var file_opts = conf.file_opts;

    if (! file_opts)
	return;

    if (file_opts.format != 'csv') {
	console.warn('only format "csv" is allowed for file logging');
	return;
    }
    ...
    require('fs').appendFile(file_opts.path, data, function(err) {
	if (err)
